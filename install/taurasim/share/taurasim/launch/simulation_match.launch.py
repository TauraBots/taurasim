...
def generate_launch_description():
    ...
    declare_robots = DeclareLaunchArgument(
        'ROBOTS',
        default_value= PythonExpression(["2 * ", LaunchConfiguration('robots_per_team')]),
        description='Número total de robôs (2 * robots_per_team)'
    )
    ...
    keyboard_group = GroupAction(
        ...
        actions=[
            Node(
                ...
                output='screen',
                parameters=[{'ROBOTS': LaunchConfiguration('ROBOTS')}]
            )
        ]
    )
